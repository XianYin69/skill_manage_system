...
def adapters():
    d = dict(ADAPTERS)
    for k, v in (resolve_home.conf(SMS).get("agent_cli") or {}).items():
        if not k.startswith("_") and isinstance(v, dict): d[k] = v
    return d
def detected():
...
def _put(name, val):
    os.makedirs(STATE, exist_ok=True); open(os.path.join(STATE, name), "w", encoding="utf-8").write(val)
def current():
    cur = _state("current_agent"); det = detected()
    return cur if cur in det else (next(iter(sorted(det)), None))
def prefix_on():
    return _state("skill_prefix", "on") != "off"
def use(name):
    _put("current_agent", name)
    return "切到 agent：" + name + ("" if name in detected() else "（未检出其 CLI——配置 agent_cli {bin,args} 并确保在 PATH）")
def skill(on):
    _put("skill_prefix", "on" if on else "off")
    return "skill_manage_system 前缀：" + _state("skill_prefix", "on")
def ask(text, on_line):
    ...
    if not ag:
        ...
        return None
    spec = adapters()[ag]
    prompt = (SKILL_DIRECTIVE if prefix_on() else "") + text
    args, env = list(spec.get("args", [])), dict(os.environ, PYTHONIOENCODING="utf-8")
    if spec.get("prompt_stdin"):
        p = subprocess.Popen([spec.get("bin", ag)] + args, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                             text=True, encoding="utf-8", errors="replace", env=env)
        p.stdin.write(prompt); p.stdin.close()
    else:
        p = subprocess.Popen([spec.get("bin", ag)] + args + [prompt], stdout=subprocess.PIPE, stderr=subprocess.STDOUT,
                             text=True, encoding="utf-8", errors="replace", env=env)
