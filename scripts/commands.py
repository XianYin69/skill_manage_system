...
if __name__ == "__main__":
    ...
    elif cmd in ("sandbox", "privacy"): r = {"script": f"scripts/{cmd}.py", "args": argv[2:], "root": os.path.join(sms, "privacy") if cmd == "privacy" else os.path.join(sms, "tmp", "sandbox")}
    else: r = {"error": "用法: help | intent <utterance> | show <name> | use <name> [args] | temp <rel> [--mkdir] | sandbox <subcommand> [args] [--write] | privacy <notice|collect|open> [args]"}
