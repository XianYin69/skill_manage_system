...
Agent 工具的"技能操作系统"：像 OS 调度进程一样发现、打包、连接、拆分、并发调度、进程注册并调度技能（子技能运行完回到 SMS），支持按天缓存清理、上下文自动压缩、本地/云端安装（下载须确认）、多客户端同步与信任链审查，并暴露时区地区、正反双辩论、删除 skill 与命令系统（help/intent/show/use）接口。
...
- [`scripts/`](scripts/scripts.md)：25 个 Python 脚本（英文名、均 ≤ 50 行，含 sandbox）。
- [`schemas/`](schemas/register.schema.json)：十五份 JSON 数据契约（含 sandbox）。
