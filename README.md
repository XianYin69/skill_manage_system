...
Agent 工具的"技能操作系统"：像 OS 调度进程一样发现、打包、连接、拆分、并发调度、进程注册并调度技能，支持按天缓存清理、上下文自动压缩、本地/云端安装、多客户端同步与信任链审查。
...
- [`scripts/`](scripts/scripts.md)：20 个 Python 脚本（英文名、均 ≤ 50 行）。
- [`schemas/`](schemas/register.schema.json)：十一份 JSON 数据契约。
