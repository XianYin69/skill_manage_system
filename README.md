...
- [`scripts/`](scripts/scripts.md)：40 个 Python 脚本（英文名、均 ≤ 50 行，含 sandbox/privacy/deps/privacy_cat/skill_cache/user_commands/shell×4/agent_stream/deploy/hud/redlines）。
- [`bin/`](bin/sms-shell)：部署包本体＝sms-shell(.cmd)＋[locate.py](bin/locate.py)（相邻→SMS_SKILL→sms_skill 三级定位回源）；部署＝仅把这些文件复制到指定路径，目标处直接运行。
...
python -B scripts/deploy.py D:\agents --write   # 部署＝仅复制 bin 启动文件到 D:\agents，目标处直接跑 sms-shell(.cmd)
```
...
- 悬空链接 = 0；所有 .md / 脚本 ≤ 50 行；SKILL.md 含 YAML frontmatter；写盘经 emit 门控：默认预览，`--write` 且已授予 write 才落盘；高危操作先询问 + `grant danger`（敏感键）；约束持久化＝AGENTS.md 镜像 + redlines.py 机械断言 + seal 基线（resistance #16）；部署＝仅复制 bin 文件到指定路径。