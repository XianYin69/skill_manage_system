...
3. **部署＝仅复制 bin 文件**：把 `bin/` 下统一入口文件（sms-shell(.cmd)·locate.py·sms_formats.py——格式 API 已并入 sms-shell，`api` 子命令/`:api`）复制到用户指定路径即完成部署；绝不整包复制 skill 本体（2026-09-24 事故教训，resistance #16）。
