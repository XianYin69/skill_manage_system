...
class Store:
    ...
    def prune(self, days=14, min_freq=1):
        ...
        dead = [(self._wj(self._p(f["chain"], f["id"]), dict(f, dead=True, pruned=cut(self.cc.get(f["chain"], {})))) or 1)
                for f in self.all_frags() if f["chain"] != "knowledge" and f["ts"][:10] < cut(self.cc.get(f["chain"], {})) and f.get("freq", 1) <= self.cc.get(f["chain"], {}).get("min_freq", min_freq)]
        return len(dead)
