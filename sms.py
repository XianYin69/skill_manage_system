#!/usr/bin/env python3
"""sms.py — 根入口程序（Windows/macOS/Linux，纯标准库）：①开箱即用：解析/创建 SMS_HOME、播种用户配置、红线自检（失败即停）；②依赖嗅探与修补：python/PySide6/git/agent CLI 逐项报告，注册表产物缺失或 --rebuild 时重建（emit 门控），--install-deps 经同意才 pip 装 PySide6；③引导至 CLI：交棒 skill/scripts/shell.py（--gui/--tui 透传）。用法：python sms.py [doctor|shell] [--rebuild] [--install-deps] [shell 参数…]；doctor 只诊断不启动。"""
import importlib.util, json, os, shutil, subprocess, sys
ROOT = os.path.dirname(os.path.abspath(__file__))
S = os.path.join(ROOT, "skill", "scripts")
sys.path.insert(0, S)
import resolve_home, agent_stream
